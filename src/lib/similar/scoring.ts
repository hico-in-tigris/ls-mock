...
function joinNonEmpty(values: readonly (string | undefined)[]): string {
  return values
...
function fallbackSummary(project: Project): string {
...
function caseTags(candidate: SimilarCase): string[] {
  return uniqueTokens((candidate.tags ?? []).map(toLower));
}

function buildProjectQuery(project: Project): string {
...
function buildCaseDocument(candidate: SimilarCase): string {
  return joinNonEmpty([
    candidate.title,
    candidate.summary,
...
interface Bm25LikeResult {
  readonly score: number;
  readonly matchedTokens: readonly string[];
}

function bm25Like(queryTokens: readonly string[], documentTokens: readonly string[]): Bm25LikeResult {
...
interface CosineLikeResult {
  readonly score: number;
  readonly textOverlap: number;
...
function intersectTokens(a: Set<string>, b: Set<string>): string[] {
...
function cosineLike(
  ...
  projectTagTokens: readonly string[],
  caseTagTokens: readonly string[],
): CosineLikeResult {
  const querySet = new Set(queryTokens);
...
function compareLoose(a: string | undefined, b: string | undefined): boolean {
  if (!a || !b) {
    return false;
  }
...
function durationIsClose(projectDuration: string | undefined, caseDuration: string | undefined): boolean {
...
function budgetIsClose(projectBudget: string | undefined, caseBudget: string | undefined): boolean {
...
function hasKpi(candidate: SimilarCase): boolean {
...
function isRecent(candidate: SimilarCase): boolean {
  if (!candidate.year) {
    return false;
  }
...
function createComponent(key: ScoreComponent['key'], weight: number, label: string): ScoreComponent {
  return { key, weight, label };
}

function ruleBoost(project: Project, candidate: SimilarCase): RuleBoostResult {
  let score = 0;
  const components: ScoreComponent[] = [];

  const purposeOverlap = Math.max(
    overlapScore(project.purpose, candidate.purpose),
    overlapScore(project.metadata?.purpose, candidate.purpose),
  );
  ...
  const audienceOverlap = Math.max(
    overlapScore(project.audience, candidate.audience),
    overlapScore(project.metadata?.audience, candidate.audience),
  );
...
function buildBreakdown(
  bm25: Bm25LikeResult,
  cosine: CosineLikeResult,
  rule: RuleBoostResult,
): SimilarCaseScoreBreakdown {
  const components: ScoreComponent[] = [...rule.components];
...
function explanationFromComponents(components: readonly ScoreComponent[]): string[] {
...
export function scoreSimilarCases(
  project: Project,
  candidates: readonly SimilarCase[],
...
export function buildProjectQueryText(project: Project): string {
